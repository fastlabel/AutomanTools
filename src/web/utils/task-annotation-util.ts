...
import { AnnotationClassVO, TaskAnnotationOriginVO, TaskAnnotationVO } from "../types/vo";
...
export const TaskAnnotationUtil = {
    ...
    merge: (voList: TaskAnnotationOriginVO[], classVoList: AnnotationClassVO[]): TaskAnnotationVO[] => {
        const classVoObj = classVoList.reduce<{ [key: string]: AnnotationClassVO }>((r, c) => {
            r[c.id] = c;
            return r;
        }, {});
        return voList.map(vo => {
            const { type, title, value, color, defaultSize } = classVoObj[vo.annotationClassId];
            return { ...vo, type, title, value, color } as TaskAnnotationVO;
        });
    },
    copyFramePoints: (voList: TaskAnnotationVO[], frameNo: { source: string, dist: string }, targetId?: string): TaskAnnotationVO[] => {
        const now = new Date();
        return voList.map(vo => {
    ...
    findNearestFramePoints: (vo: TaskAnnotationVO, frameNo: string) => {
        const res = Object.keys(vo.points).sort().reduce<{ prev: string, next: string }>((r, f) => {
            if (f < r.prev) {
                r.prev = f;
            }
            if (f > r.next) {
