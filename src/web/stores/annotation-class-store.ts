...
const reducer: () => Reducer<AnnotationClassState, ClassesAction> = () => {
    return (state, action) => {
        switch (action.type) {
            case 'init':
                if (state.status !== 'none' && state.status !== 'saved') throw new Error(`illegal state ${JSON.stringify(state)}`);
                return { status: 'ready', projectId: action.projectId, data: action.data };
            case 'add':
                if (state.status !== 'ready') throw new Error(`illegal state ${JSON.stringify(state)}`);
            ...
            case 'remove':
                if (state.status !== 'ready') throw new Error(`illegal state ${JSON.stringify(state)}`);
                const removed = state.data.filter(c => c.id !== action.annotationClassId);
                return { ...state, data: removed };
            case 'fetch':
                if (state.status === 'saving') throw new Error(`illegal state ${JSON.stringify(state)}`);
                return { ...state, status: 'loading', projectId: action.projectId };
            case 'fetched':
                if (state.status !== 'loading') throw new Error(`illegal state ${JSON.stringify(state)}`);
                return { ...state, status: 'ready', data: action.data };
            case 'save':
                if (state.status !== 'ready') throw new Error(`illegal state ${JSON.stringify(state)}`);
                return { ...state, status: 'saving', callbackSaved: action.callbackSaved };
            case 'saved':
                if (state.status !== 'saving') throw new Error(`illegal state ${JSON.stringify(state)}`);
                return { ...state, status: 'saved' };
            case 'end':
                if (state.status !== 'saved') throw new Error(`illegal state ${JSON.stringify(state)}`);
                return { status: 'none' };
        }
    }
};
...
const useAnnotationClass = () => {
    ...
    const [annotationClass, dispatchAnnotationClass] = useReducer(reducer(), { status: 'none' });

    useEffect(() => {
        if (annotationClass.status === 'loading') {
            projectRepository.loadAnnotationClasses(annotationClass.projectId).then(res => {
                dispatchAnnotationClass({ type: 'fetched', data: res.annotationClasses });
            });
        } else if (annotationClass.status === 'saving') {
            projectRepository.saveAnnotationClasses({
    ...
    return {
        annotationClass, dispatchAnnotationClass
    };
}
