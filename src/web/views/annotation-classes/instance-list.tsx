...
type Props = {
    ...
    onClickItem?: (item: TaskAnnotationVO, mode?: 'add' | 'remove' | 'single' | 'clear') => void;
...
const InstanceList: FC<Props> = ({ frameNo, instances, editingTaskAnnotation, invisibleClasses, selectedItems, onClickItem = f => f, onClickToggleInvisible }) => {
    const styleClasses = useStyles();
    const getClassTagStyle = (type: AnnotationType): any => {
        return styleClasses.markCuboid;
    };

    const collapseContent = useMemo(() => (item: TaskAnnotationVO) => {
        const [positionX, positionY, positionZ,
            rotationX, rotationY, rotationZ,
            width, height, deps] = item.points[frameNo];
        const data = {
        ...
        return (
            <Collapse in={true} timeout={100} unmountOnExit>
                <List disablePadding>
                    <ListItem dense>
                        <FLTextField mode='list' readonly={true} label={"座標 X"} form={['positionX', formObj]} />
                    </ListItem>
                    <ListItem dense>
                        <FLTextField mode='list' readonly={true} label={"座標 Y"} form={['positionY', formObj]} />
                    </ListItem>
                    <ListItem dense>
                        <FLTextField mode='list' readonly={true} label={"座標 Z"} form={['positionZ', formObj]} />
                    </ListItem>
                    <ListItem dense>
                        <FLTextField mode='list' readonly={true} label={"回転 X"} form={['rotationX', formObj]} />
                    </ListItem>
                    <ListItem dense>
                        <FLTextField mode='list' readonly={true} label={"回転 Y"} form={['rotationY', formObj]} />
                    </ListItem>
                    <ListItem dense>
                        <FLTextField mode='list' readonly={true} label={"回転 Z"} form={['rotationZ', formObj]} />
                    </ListItem>
                    <ListItem dense>
                        <FLTextField mode='list' readonly={true} label={"幅"} form={['width', formObj]} />
                    </ListItem>
                    <ListItem dense>
                        <FLTextField mode='list' readonly={true} label={"高さ"} form={['height', formObj]} />
                    </ListItem>
                    <ListItem dense>
                        <FLTextField mode='list' readonly={true} label={"長さ"} form={['deps', formObj]} />
                    </ListItem>
                </List>
            </Collapse>);
    }, []);

    const listItemRenderer = useMemo(() => (item: TaskAnnotationVO, content?: (item: TaskAnnotationVO) => JSX.Element) => {
        const selected = selectedItems?.has(item.id) === true;
        const hidden = invisibleClasses?.has(item.id);

        return <React.Fragment key={item.id}>
            <ListItem button dense selected={selected} onClick={(event) => onClickItem(item, resolveMode(selected, event))}>
