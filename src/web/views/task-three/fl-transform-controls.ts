import {
    BufferGeometry, Camera, DoubleSide, Intersection, Material, Matrix4, Mesh, MeshBasicMaterial,
    Object3D, PlaneGeometry, Quaternion, Raycaster, Vector3
} from 'three';
import { FLOrbitControls } from './fl-orbit-controls';
import { ControlKey, ControlType, FLTransformControlsGizmo } from './fl-transform-controls-gizmo';
...
class FLTransformControls<TCamera extends Camera = Camera> extends Object3D {
    ...
    constructor(camera: TCamera, domElement: HTMLElement, control: ControlType) {
        ...
        if (domElement === undefined) {
            console.warn('THREE.FLTransformControls: The second parameter "domElement" is now mandatory.')
    ...
    public updateMatrixWorld = (): void => {
        if (this.object !== undefined) {
            ...
            if (this.object.parent === null) {
                console.error('FLTransformControls: The attached 3D object must be a part of the scene graph.')
            } else {
    ...
    private pointerHover = (pointer: FLTransformControlsPointerObject): void => {
        ...
        const intersect = this.intersectObjectWithRay(this.gizmo.getPicker(), this.raycaster);
        if (intersect) {
