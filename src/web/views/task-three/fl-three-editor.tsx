...
const FLThreeEditor: FC<Props> = ({ frameNo, annotations, selectable, showLabel, cubeGroupRef, bgMain, bgSub, position0, targets, preObject, cameraHelper, onClickObj = f => f, onPutObject = f => f, onObjectChange = f => f }) => {
    ...
    useEffect(() => {
        if (rootRef.current) {
            ...
            const handleResize = () => {
                let canvas = root.getElementsByTagName('canvas') as any;
    ...
    return (
        <div className={styles.root} ref={rootRef}>
            <Box flexGrow={1} mt={2} mr={2} ml={2} mb={1}>
                <Canvas camera={{ fov: 90, up: new Vector3(0, 0, 1), rotation: new Euler(0, 0, 0, 'ZXY') }} style={{ backgroundColor: 'black' }} resize={C_RESIZE}>
                    <FLMainControls position0={position0} preObject={preObject} onPutObject={onPutObject} />
                    {bgMain}
                    <FLCubes ref={_cubeGroupRef} selectable={selectable} showLabel={showLabel} frameNo={frameNo} annotations={annotations} onClick={onClickObj} />
                    {cameraHelper}
                </Canvas>
            </Box>
            <Box mr={2} ml={2} mb={1}>
                <Grid container spacing={2} className={styles.footer}>
                    <Grid item xs={4} >
                        ...
                        <Box height={footerBoxH}>
                            <Canvas orthographic camera={{ near, far, zoom, up: new Vector3(1, 0, 0) }} style={{ backgroundColor: 'black' }} resize={C_RESIZE}>
                                {bgSub}
                    ...
                    <Grid item xs={4}>
                        ...
                        <Box height={footerBoxH}>
                            <Canvas orthographic camera={{ near, far, zoom, up: new Vector3(0, 0, 1), position: new Vector3(0, -1, 0) }} style={{ backgroundColor: 'black' }} resize={C_RESIZE}>
                                {bgSub}
                    ...
                    <Grid item xs={4}>
                        ...
                        <Box height={footerBoxH}>
                            <Canvas orthographic camera={{ near, far, zoom, up: new Vector3(0, 0, 1), position: new Vector3(1, 0, 0) }} style={{ backgroundColor: 'black' }} resize={C_RESIZE}>
                                {bgSub}
